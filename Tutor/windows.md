...
> **Note:** The captcha web solver uses **Selenium** with Chrome (installed automatically via `webdriver-manager`). If you want browser-based captcha solving, make sure Chrome is installed on your system.
...
Run the setup wizard  it'll walk you through everything:
...
Create a file called `.env` in the bot folder and add your tokens:

```env
TOKENS="YOUR_DISCORD_TOKEN CHANNEL_ID;TOKEN2 CHANNEL_ID2"
```
...
- Back up your `.env` and config files somewhere safe, just in case
