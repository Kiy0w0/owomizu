...
class Blackjack(commands.Cog):
        ...
        try:
            ...
            if (
                ...
                if not self.gamble_flags["no_balance"]:
                    ...
                    await self.bot.log(
                        f"Amount to gamble next ({amount_to_gamble}) exceeds bot balance ({self.bot.user_status.get('balance',0)}), stopping blackjack!",
                        "#4a270c",
                    )
                    notify(
                        f"Amount to gamble next ({amount_to_gamble}) exceeds bot balance, stopping blackjack!",
                ...
                await self.bot.log(
                    f"Balance regained! ({self.bot.user_status.get('balance',0)}) - restarting blackjack!",
                    "#4a270c",
                )
            ...
            await self.bot.log(f"Error in send_blackjack: {e}", "#c25560")
    ...
    async def cog_load(self):
        if not self.bot.settings_dict["gamble"]["blackjack"].get("enabled", False):
            ...
            await self.bot.log("Blackjack Auto-Gamble Started! 🎰", "#51cf66")
